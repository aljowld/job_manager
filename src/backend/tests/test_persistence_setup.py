...
def test_core_models_are_registered() -> None:
    model_names = {model.__tablename__ for model in (JobSource, RawJobSnapshot, JobOffer, JobSourceOccurrence)}

    assert {"job_sources", "raw_job_snapshots", "job_offers", "job_source_occurrences"} <= model_names


def test_metadata_contains_expected_columns() -> None:
    job_offer_columns = inspect(Base.metadata.tables["job_offers"]).columns.keys()

    assert {"id", "title", "company_name", "description", "created_at", "updated_at"}.issubset(job_offer_columns)
